...
function Header() {
  ...
  const navItems = [
    ...
    { label: 'Find Tutor', path: '/findtutor' },
    ...
    >
      <Toolbar sx={{ px: { xs: 2, sm: 3 }, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <IconButton
            ...
            sx={{ mr: 2, display: { xs: 'flex', '@media (min-width: 1150px)': { display: 'none' } } }}
          >
            <MenuIcon />
          </IconButton>
        ...
        <Box sx={{ display: { xs: 'none', '@media (min-width: 1150px)': { display: 'block' } } }}>
          {navItems.map((item) => (
            <Button
              key={item.path}
              color="inherit"
      ...
      <Box component="nav">
        <Drawer
          ...
          sx={{
            display: { xs: 'block', '@media (min-width: 1150px)': { display: 'none' } }
          }}
        >
          {drawer}
        </Drawer>
