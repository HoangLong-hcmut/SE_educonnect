...
function Header() {
  ...
  const navItems = [
    ...
    { label: 'Reports & Stats', path: '/admin/statistics' },
    { label: 'Tutor Performance', path: '/admin/tutorperformance' },
    { label: 'Profile Approval', path: '/admin/approval' },
    ...
    >
      <Toolbar sx={{ px: { xs: 2, sm: 3 }, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <IconButton
            ...
            sx={{ mr: 2, display: { xs: 'flex', '@media (min-width: 1440px)': { display: 'none' } } }}
          >
            <MenuIcon />
          </IconButton>
        ...
        <Box sx={{ display: { xs: 'none', '@media (min-width: 1440px)': { display: 'block' } } }}>
          {navItems.map((item) => (
            <Button
              key={item.path}
              onClick={() => navigate(item.path)}
      ...
      <Box component="nav">
        <Drawer
          ...
          sx={{
            display: { xs: 'block', '@media (min-width: 1440px)': { display: 'none' } }
          }}
        >
          {drawer}
        </Drawer>
